    ...
    <div v-if="filterData" class="filter_wrap">
      ...
      <aside class="filter">
        <div class="filter-nav" 
          v-bind:class="{ 'filter-bold' : currentIndex == index}"
    ...
    <section class="filter-extend" v-if="isSort">
      <ul>
        <li v-for="(item,index) in filterData.sortBy" :key="index">
          <span :class="{ selectName: currentSort === index}" @click="selectSort(item,index)">{{item.name}}</span>
...
export default {
  ...
  methods: {
    ...
    selectSort (item,index) {
      ...
      // 以下三行是自己想的实现点击更改排名字符串 改变dom渲染出来的数据
      console.log(document.getElementsByClassName('filter-nav')[0].innerText);
      console.log(item.name);
      document.getElementsByClassName('filter-nav')[0].innerText = item.name
      // 以下是老师的 改变vue的数据源
      this.filterData.navTab[0] = this.filterData.sortBy[index].name; // 直接使用 item.name在页面重新加载时会出现空字符串
      this.hideView();
